...
:: Check if portable Python setup is needed
set "SETUP_PYTHON=Y"
if exist "portable_python.tar" (
    echo.
    set /p SETUP_PYTHON="Portable Python archive already exists. Recreate it? (Only needed if Python dependencies changed) (Y/N): "
)

:: Setup portable Python only if needed
if /i "%SETUP_PYTHON%"=="Y" (
    echo Setting up portable Python...
    call portable_python_setup.bat
    
    :: Create portable Python archive using tar
    ...
    echo Skipping portable Python setup, using existing archive...
    copy /Y "portable_python.tar" "%NETWORK_DIST%\"
)
...
echo Creating extraction script for portable Python...
...
if /i "%DEPLOY_NETWORK%"=="Y" (
    ...
    :: Only ask about Python deployment if we created a new archive or the network doesn't have one
    if /i "%SETUP_PYTHON%"=="Y" (
        ...
        echo.
        echo Skipping portable Python archive deployment (using existing archive).
    )
    ...
    if /i "%DEPLOY_PYTHON%"=="Y" (
        echo.
        echo Copying portable Python archive...
